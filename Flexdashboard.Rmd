...
sheets_auth(email = "blueberripie@gmail.com")

# might need to update the widgets where possible
# shinyWidgets::shinyWidgetsGallery()

# why shiny not loaded?
library(shiny)
```
...
textInput("googlesheets_id_entry", 
          "Googlesheets ID", 
          value = "1LJUpvwV5syRrsRXd8Go2qExvRrZgYQVXDPtDx3WWPU0") 
# UPDATE WITH UNIQUE ID from d/ ..... /edit in url
...
bank <- reactive(if (is.null(input$bank)){
  
  blank_table
  
} else {
  
  read_csv(input$bank$datapath, col_names = F) %>% 
    rename(Date = 1, Amount = 2, Description = 3, Balance = 4) %>%
    mutate(data_source = "bank",
           Date = dmy(Date),
           Amount = as.numeric(Amount)) %>% # giving Error about Amount data type
    select(-Balance)
})
...
creditcard <- reactive(if (is.null(input$creditcard)){
  
  blank_table
  
} else {
  
  read_csv(input$creditcard$datapath, col_names = F) %>% 
    rename(Date = 1, Amount = 2, Description = 3) %>%  #, Balance = 4) %>%
    mutate(data_source = "creditcard",
           Date = dmy(Date),
           Amount = as.numeric(Amount)) #%>% # giving Error about Amount data type
  #  select(-Balance)
})
...
combined_import1 <- reactive(bind_rows(bank(), 
                                       creditcard(), 
                                       paypal(),
                                       # cash(),
                                       # up_bank()
